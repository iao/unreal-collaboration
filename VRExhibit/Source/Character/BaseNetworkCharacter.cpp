...
// Set the meshes when server replicates it
void ABaseNetworkCharacter::OnRep_Hair() {
	if (!HairMesh->SkeletalMesh) HairMesh->SetSkeletalMesh(Hair);
}

void ABaseNetworkCharacter::OnRep_Head() {
	if (!HeadMesh->SkeletalMesh) HeadMesh->SetSkeletalMesh(Head);
}

void ABaseNetworkCharacter::OnRep_Chest() {
	if (!ChestMesh->SkeletalMesh) ChestMesh->SetSkeletalMesh(Chest);
}

void ABaseNetworkCharacter::OnRep_Arm() {
	if (!ArmMesh->SkeletalMesh) ArmMesh->SetSkeletalMesh(Arm);
}

void ABaseNetworkCharacter::OnRep_Leg() {
	if (!LegMesh->SkeletalMesh) LegMesh->SetSkeletalMesh(Leg);
}
...
void ABaseNetworkCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
	...
	DOREPLIFETIME(ABaseNetworkCharacter, Hair);
	DOREPLIFETIME(ABaseNetworkCharacter, Head);
	DOREPLIFETIME(ABaseNetworkCharacter, Chest);
	DOREPLIFETIME(ABaseNetworkCharacter, Arm);
	DOREPLIFETIME(ABaseNetworkCharacter, Leg);
