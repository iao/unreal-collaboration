...
void AHTTPService::KeepAlive(FString URL, FKeepAliveStruct keepalive, AMyPlayerController* controller) {
	...
	Request->OnProcessRequestComplete().BindUObject(controller, &AMyPlayerController::KeepAliveResponce);
	Send(Request);
}

void AHTTPService::Info(FString URL, FInfoStruct_Request info, AMyPlayerController* controller) {
	FString ContentJsonString;
	GetJsonStringFromStruct<FInfoStruct_Request>(info, ContentJsonString);

	TSharedRef<IHttpRequest> Request = PostRequest(URL, ContentJsonString);
	Request->OnProcessRequestComplete().BindUObject(controller, &AMyPlayerController::InfoResponce);
	Send(Request);
}