...
void AHTTPService::KeepAlive(FString URL, FKeepAliveStruct keepalive, ANetworkPlayerController* controller) {
	...
	Request->OnProcessRequestComplete().BindUObject(controller, &ANetworkPlayerController::KeepAliveResponse);
	Send(Request);
}
...
void AHTTPService::Info(FString URL, FInfoStruct_Request info, ANetworkCharacter* character) {
	...
	TSharedRef<IHttpRequest> Request = PostRequest(URL, ContentJsonString);
	Request->OnProcessRequestComplete().BindUObject(character, &ANetworkCharacter::InfoResponse);
	Send(Request);
}
...
void AHTTPService::Time(FString URL, FTimeStruct_Request time, ANetworkPlayerController* controller) {
	...
	TSharedRef<IHttpRequest> Request = PostRequest(URL, ContentJsonString);
	Request->OnProcessRequestComplete().BindUObject(controller, &ANetworkPlayerController::TimeResponse);
	Send(Request);
}