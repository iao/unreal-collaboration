...
void ASignPawn::BeginPlay() {
	...
	if (GetLocalRole() != ROLE_Authority) {
		...
		if (controller) SetActorHiddenInGame(Cast<ABaseNetworkPlayerController>(controller)->isHidden);
	}
}
...
void ASignPawn::OnRep_TextActor() {
	...
	if (GetLocalRole() != ROLE_Authority) {
		APlayerController* controller = GetWorld()->GetFirstPlayerController();
		if (controller && TextActor) TextActor->HideActor(Cast<ABaseNetworkPlayerController>(controller)->isHidden);
	}
}
